...
class Game {
    constructor(vsComputer = false){
        ...
        this._ready = false;
        this.loadPlayerWins()
        this.buttonClickSound = new Audio('audio/click.mp3');
        this.moveSound = new Audio('audio/move.mp3');
        this.gameOverSound = new Audio('audio/gameover.wav');
        this.gameOverSound2 = new Audio('audio/gameover2.wav');
        this.buttonClickSound.volume = 0.1;
    ...
    // /**
    //  * Load player wins from local storage
    //  */
    loadPlayerWins() {
        ...
        if (player1Wins !== null) {
          this.players[0].wins = parseInt(player1Wins);
        } else {
          this.players[0].wins = 0;
        }
      
        if (player2Wins !== null) {
          this.players[1].wins = parseInt(player2Wins);
        } else {
          this.players[1].wins = 0;
        }
      }
    /**
     * Save player wins to local storage
     */
    savePlayerWins() {
        localStorage.setItem('player1Wins', this.players[0].wins);
        localStorage.setItem('player2Wins', this.players[1].wins);
    }
    ...
    gameOver(message, result) {
        ...
        if (result === 'draw') {
            ...
            winnerTally.textContent = ++this.activePlayer.wins;
            this.savePlayerWins(); // Save player wins to local storage
            this.gameOverSound.play();
        }
    ...
    startGame(){
        ...
        this.loadPlayerWins(); // Load player wins from local storage
        this.playerTurn();
    }
}